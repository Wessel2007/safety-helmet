...
- [ ] Criar repositório no GitHub (ex: `helmet-detection-yolo` ou `epi-detector`)
- [ ] Criar estrutura de pastas local:
  ...
  └── README.md
  ```
- [ ] Criar `.gitignore` (ignorar `venv/`, `*.pt` pesados, `data/raw/`, `__pycache__/`, `.ipynb_checkpoints/`)
- [ ] `git init` + primeiro commit: **"chore: estrutura inicial do projeto"**

### 2. Ambiente virtual e dependências
- [ ] Criar venv: `python -m venv venv` (não precisa admin)
- [ ] Ativar venv e instalar: `pip install ultralytics opencv-python matplotlib pandas`
- [ ] Gerar `requirements.txt`: `pip freeze > requirements.txt`
- [ ] Testar se `ultralytics` importa sem erro (`python -c "import ultralytics"`)
- [ ] Commit: **"chore: configura ambiente virtual e dependências"**

### 3. Obtenção do dataset
- [ ] Buscar dataset público pronto de detecção de capacete (ex: "Hard Hat Workers Dataset" no Kaggle, ou opções no Roboflow Universe já anotadas em formato YOLO)
- [ ] Baixar e extrair em `data/raw/`
- [ ] Verificar que o dataset já vem no formato YOLO (imagens + `.txt` com bounding boxes) ou anotar a estrutura de classes que ele usa (ex: `helmet`, `no-helmet`, `person`)
- [ ] Documentar a fonte do dataset (link, licença) em `docs/dataset.md` — importante para dar crédito e mostrar profissionalismo
